        ...
        if op_choice < cxpb:  # Apply crossover
            ...
            if ind1 is not None:
                ...
                try:
                    ...
                    print("Error in mutation, adding old pipeline now")
                    ind1 = toolbox.clone(population[np.random.randint(0, len(population))])
                # END ADDITIONAL FS code
            ...
            try:
                ...
                print("Error in mutation, adding old pipeline now")
                ind = toolbox.clone(population[np.random.randint(0, len(population))])
            # END ADDITIONAL FS code
    ...
    try:
        with warnings.catch_warnings():
            ...
            if not fs_modifier is None:
                ...
                print("The final score becomes %f, after multiplying it it by %.2f  for %i features and modifier %.2f"
                      % (np.average(np.asarray(scores)), fs_modifier ** n_features, n_features, fs_modifier))
            # END ADDITIONAL FS code
