...
# NEW IMPORTS FS selection
from.config.feature_selection import fs_config, order_config, fs_pipeline_steps
from .builtins import mutation_insert
from .gp_types import Input_Array
...
class TPOTBase(BaseEstimator):
    ...
    def _setup_config(self, config_dict):
        if config_dict:
            ...
            elif config_dict == 'TPOT FS':
                if self.classification:
                    self.config_dict = high_FS_classifier_config_dict
                else:
    ...
    def fit(self, features, target, sample_weight=None, groups=None):
        ...
        # START ADDITIONAL FS code
            ...
            print("Unknown non-base exception caused the system to stop")
        # END ADDITIONAL FS code
    ...
    def _random_mutation_operator(self, individual, allow_shrink=True):
        ...
        if self.feature_selection:
            mutation_techniques = [
                partial(mutation_insert.mutInsert, pset=self._pset),
                partial(mutNodeReplacement, pset=self._pset)
            ]
        ...
        for _ in range(self._max_mut_loops):
            ...
            # START ADDITIONAL FS code
            mutator = np.random.choice(mutation_techniques)
            #print("%s \n unsuccessful %i times" %(str(mutator), unsuccesful_mutations + 1))
            # END ADDITIONAL FS code
